...
3. Visualize in two dimensions using by plotting the top two PCA components
...
This was directly based on the experiment code from the paper, and takes all weekdays as input prompts - I’ve called these *base items*. This list was appended with prompts prefixed with *modifiers* ”very early on” and ”very late on”, which resulted in prompts ranging from "very early on Monday" to "very late on Sunday". 
...
- The ordering of modifiers was less consistent than in the original paper - (what exactly happened?)
